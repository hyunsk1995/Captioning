...
class VisualQuestionAnswering (private val context: Context) {
    private val client = OkHttpClient()

    suspend fun askWithPrompt(bitmap: Bitmap, prompt: String): String {
        var answer = ""
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                // Parse response JSON
                answer = parseAnswerFromResponse(responseString)
                Log.d("VQA", answer)
            } catch (e: IOException) {
                e.printStackTrace()
            }
    ...
    private fun parseAnswerFromResponse(response: String?): String {
        response?.let {
            ...
            return jsonResponse.predictions?.get(0)?: "No answer available"
        }
